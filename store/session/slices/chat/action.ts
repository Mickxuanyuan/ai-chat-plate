/**
 * Chat Slice：actions（消息发送/生成/编辑等写操作）。
 *
 * 关键点：
 * - 聊天消息写入通过 `dispatchMessage -> messagesReducer` 完成（纯 reducer）
 * - AI 回复通过 SSE 流式拉取，并持续更新 assistant 消息内容
 */
...
import { type MessageDispatch, messagesReducer } from "./messageReducer";

/**
 * 占位符：用于在 SSE 开始前创建一条 assistant 消息，以便 UI 立即展示“正在生成”。
 */
const LOADING_FLAT = "...";
...
> = (set, get) => ({
  ...
  generateMessage: async (messages, options) => {
    ...
    const fetcher = () =>
      fetchChatModel({ messages, model: config.model, ...config.params });
  ...
  realFetchAIResponse: async (messages: ChatMessage[], parentId?: string) => {
    ...
    /** 本次生成对应的 assistant 消息 id */
    const assistantId = nanoid();
    /** 本次生成对应的 user 消息 id（用于 parentId 关联） */
