# 主题切换方案（ThemeProvider + data-theme + Tailwind dark）

本项目的主题切换目标是：

- 支持 `system / light / dark` 三种模式
- 不引入重量级主题库
- 用统一的 `data-theme` 驱动样式（CSS 变量 + Tailwind `dark:`）
...
3) **CSS 根据 `data-theme` 覆盖变量**

- 在 `globals.css` 里定义 `[data-theme="dark"]` / `[data-theme="light"]` 覆盖 `--background` / `--foreground` 等 CSS 变量

4) **Tailwind 的 `dark:` 变体跟随 `data-theme="dark"`**
...
- 写入 `document.documentElement[data-theme]`
...
### 2.3 Tailwind dark: 跟随 data-theme

在 Tailwind v4 中，通过 `@custom-variant` 改写 `dark:` 的生效条件：
...
含义：当元素本身或其祖先存在 `[data-theme="dark"]` 时，`dark:*` 样式生效。
...
Tailwind 默认的 `dark:` 通常只跟随系统主题或 `.dark` 类。我们这里用的是 `data-theme="dark"`，所以需要把 `dark:` 的判断条件切换到 `data-theme`。

### Q2: 可能出现首次渲染闪烁（FOUC）怎么办？

当前 ThemeProvider 在 `useEffect` 里设置 `data-theme`，极少数情况下会出现闪烁。
如果你需要“完全不闪烁”，可以在 `app/layout.tsx` 的 `<head>` 加一段很短的 inline script，尽早设置 `data-theme`。需要的话我可以帮你补一版。
